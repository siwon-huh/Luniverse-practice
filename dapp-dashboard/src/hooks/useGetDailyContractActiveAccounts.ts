...
const useGetDailyContractActiveAccounts = ({
    address,
    ...
    useQuery({
        queryKey: [
            KEYS.QUERY_HOURLY_CONTRACT_ACTIVE_ACCOUNTS,
            startDate,
            endDate,
            address,
        ],
        queryFn: () => {
            if (!address) return;
            return Luniverse.getDailyContractActiveAccounts(
                address,
                startDate,
                endDate
            );
...
export default useGetDailyContractActiveAccounts;
